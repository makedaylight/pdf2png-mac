...
//
// This version was modified to make it easier for our testing.
//
...
{
    double desiredResolution = 144; // in DPI
    ...
    {
        outputFolder = [args objectAtIndex: index + 1];
        [args removeObjectAtIndex: index + 1];
        [args removeObjectAtIndex: index];
    }
    ...
    if ( [args count] != 1 || [args indexOfObject: @"--help"] != NSNotFound || desiredResolution <= 0 || page <= 0 )
    {
        ...
        fprintf( stderr, "\t--output path\tSpecify output folder\n" );
    ...
    NSString* outputFileFormat = @"%@/page%04d";
    ...
    do
    {
        ...
        NSString *outputFilePath = [ [NSString stringWithFormat: outputFileFormat, outputFolder, page] stringByAppendingPathExtension: @"png"];
